 ...
 */
/**
 * @扩展复选框树增加参数控制是否显示复选框，default为true
 * @author renmian
 * @version 4.12  2009/02/06
 */ 
...
xyTree.TreeConfig = {
  ...
  loadingIcon     : 'js/xytree/images/loading.gif',
...
xyTree.CONSTANT = {
  ...
  /*这是节点背景色的渐变颜色*/
  //color : ['#ffff00','#ffff20','#ffff40','#ffff60','#ffff80','#ffffa0','#ffffc0','#ffffff'],
  //color : ['#ff0000','#ff0020','#ff0040','#ff0060','#ff0080','#ff00a0','#ff00c0','#ff00ff'],
  color : ['#ff0000','#ffff00','#ffff40','#ffff80','#ffffa0','#ffffff'],
  colorNum :6,
 ...
 * @param {boolean} showCheck 可选，是否构造复选框
 */
xyTree.DivTree = function(name,img,showCheck) {
  /**
   * 树的根节点图标
   * @type String
   */
  ...
  this.showCheck;
  if (img){ 
    this.img = xyTree.CONSTANT.imagesPath + '/' + img;
   }
  ...
  this.tree = new xyTree.Tree(name, objectname, showCheck);
 ...
 */
xyTree.DivTree.prototype.init = function(funClickNode,funOncontextmenu,funClickRootNode) {
  ...
  this.oncontextmenu=funOncontextmenu ? funOncontextmenu : this.defaultoncontextmenu;
...
xyTree.DivTree.prototype.initTreeForm = function() {
  ...
  this.init(this.clickNode,this.oncontextmenu,this.clickRootNode);        //重新加载DOM对象
...
/**
 * 缺省的单击右键的行为，获取文本<a></a>id
 * @param {xyTree.Node} node 被单击的节点
 */
xyTree.DivTree.prototype.defaultoncontextmenu = function(node) {
 return true;
}
...
xyTree.DivTree.prototype.creatediv = function() {
 ...
 // a.oncontextmenu =function () {alert('aaaaa');return false;} //增加右键事件 zy
...
//用户自定义图片路径
xyTree.DivTree.prototype.setImagesPath = function(inPath){
	xyTree.CONSTANT.imagesPath = inPath;
}
