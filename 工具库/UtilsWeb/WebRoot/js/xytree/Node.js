 ...
 */
 ...
 /**
 * @扩展复选框树增加参数控制是否显示复选框，default为false
 * @author renmian
 ...
 */
xyTree.Node = function(name, showCheck, id, img, clickNodeJs, rightMenu) {
  /**
   * 节点id
   * @type String
   **/
  ...
  this.showCheck = showCheck;//是否显示CheckBox。true：显示；false：不显示；缺省为不显示。
  if(this.showCheck == false)
  		this.checked = 1;
  
  this.clickNodeJs = clickNodeJs;
...
xyTree.Node.prototype.innerhtml = function() {
  ...
  a.oncontextmenu = function() {
    //current.slowChange();
   	current.tree.divtree.oncontextmenu(current);
  };
